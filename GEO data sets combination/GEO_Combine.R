rm(list = ls())
library(limma)
#To determine whether the dataset GSE7186 requires Log2+1
...
#To determine whether the dataset GSE23143 requiresLog2+1
...
expname <- "Combined_Datasets_Matrix.csv" 
groupname <- "Combined_Datasets_Group.csv" 
outputname <- "Combined_Datasets_DEGs.csv" 
groupnameA <- "Normal" 
groupnameB <- "AML" 
...
sampleinfo <- data.table::fread(groupname,data.table=F) 
...
## Linear modeling
fit <- lmFit(raw_count,design) 
cont.matrix <- makeContrasts(contrasts = c("AML-Normal"), 
                             levels = design)
